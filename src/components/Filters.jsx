  ...
  return (
    <>
      <FiltersStyle>
        ...
        {strRoute.length > 58 ? (
          <div className='marsh'>
            <div>Siz {latLongV?.length} ta manzil tanladingiz</div>
            <a href={strRoute} target='_blank'>
              Marshrutni ko'rish
            </a>
          </div>
        ) : null}
            ...
            return (
              <div className='village' key={index}>
                ...
                <div className='p2'>
                  ...
                  {!(latLongV?.filter((i) => i?.unique_id === unique_id))
                    .length > 0 ? (
                    <button
                      className='plus non'
                      onClick={() => addLatLong(unique_id, latitude, longitude)}
                    >
                      add
                    </button>
                  ) : (
                    <button
                      ...
                      onClick={() => removeLatLong(unique_id)}
                    >
                      remove
                    </button>
                  )}
                  ...
                  {/* <div
                    key={index}
                    className={
                      btn === true && !index ? 'plus active' : 'plus non'
                    }
                    onClick={() => setBtn(!btn)}
                  >
                    {btn === true && !index
                      ? 'Mashrutdan chiqarish'
                      : "Mashrutga qo'shish"}
                  </div> */}
                </div>
              </div>
