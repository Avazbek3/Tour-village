...
const Famous = () => {
  ...
  const getDetail = () => {
    setLoading(true);
    GetNotAuthInstance()
      .get(`/api/v1/village/famous-places/${slug}/`)
      .then((res) => {
  ...
  const to = `/catalog/${detail?.village.slug}`;
      ...
      {loading ? (
        ...
        <HotelStyle>
          <div className='links container'>
            ...
            <Link to={to}>\ {detail?.village.name}</Link>
          ...
          {detail?.about && detail?.about.length > 0 ? (
            <div className='container'>
              <div className='information'>
                <div className='h1'>Joy haqida</div>
                <div
                  className={textHide === true ? 'h2 h2non' : ' h2 h2active'}
                >
                  {detail?.about}
                  {detail?.about.length >= 1000 ? <img src={opac} /> : null}
                </div>
                {detail?.about.length >= 1000 ? (
                  <div
                    className='stretch'
          ...
          {/* {detail?.village_history && detail?.village_history.length > 0 ? (
            <div className='container'>
              <div className='information'>
          ...
          {detail?.nearby_hotels && detail?.nearby_hotels.length > 0 ? (
            <Hotels
              village={detail?.nearby_hotels}
              h3='Atrofdagi mexmonxonalarlar'
              sub_slug={'restaurant'}
